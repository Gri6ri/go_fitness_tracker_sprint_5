...
// Удалена константа MinInHours = 60 (количество минут в одном часе) за ненадобностью. В формулах использована функция Duration.Minutes()
const (
...
// Не нужна константа SwimmingLenStep = 1.38 (длина одного гребка)
// Данная константа используется для вычисления distance (дистанции, которую преодолел пользователь)
// но вычисляя по формуле (количество_повторов * длина_шага / м_в_км) для примера swimming в func main()
// получаем (2000 * 1.38 / 1000) = 2.76 км.
// что не равно (длина_бассейна * количество_пересечений / м_в_км) = 5 * 50 / 1000 = 0.25 км
